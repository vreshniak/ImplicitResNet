	...
	\foreach \th in {0.00, 0.50, 1.00}
	{
		\begin{tikzpicture}
			\begin{axis} [
					...
					tick style={draw=none},
					very thick ]
					\addplot graphics [points={(0,0) (3,2)}, includegraphics={trim=0 0 0 0,clip}] {theta_\th_spectrum.pdf};
			\end{axis}
		\end{tikzpicture}
	}
...
\begin{figure}[t]
	\centering
	\ref{named0.00}\\
	\foreach \file in {learned_solution_20periods, learned_ode_solution_20periods}
	{
		\foreach \th/\shift in {0.00/-1, 0.50/0, 1.00/1}
		{
			\begin{tikzpicture}[every mark/.append style={mark size=1pt}]
				\begin{axis} [
						...
						\ifthenelse{ \equal{\file}{learned_ode_solution_20periods} \and \equal{\th}{0.00} }{
