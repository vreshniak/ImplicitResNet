...
def get_system(name, n, dtype, device, batches=_batches):
	if name=='wilkinson':
		# https://www.mathworks.com/help/matlab/ref/wilkinson.html
		A = torch.zeros((batches,n,n)).to(dtype=dtype, device=device)
		...
		torch.diagonal(A, offset=1,  dim1=1, dim2=2)[...] = torch.ones_like(torch.diagonal(A, offset=1, dim1=1, dim2=2))[...]
		x  = torch.ones((batches,n)).to(dtype=dtype, device=device)
		# x0 = x + 0.1*torch.randn_like(x)
		x0 = torch.rand((batches,n)).to(dtype=dtype, device=device)
		# x0 = torch.zeros((batches,n)).to(dtype=dtype, device=device
	...
	if name=='laplace':
		...
		A  = torch.rand((batches,n,n)).to(dtype=dtype, device=device)
		x0 = torch.rand((batches,n)).to(dtype=dtype, device=device)
		...
		input = torch.ones((batches,n), dtype=dtype, device=device).requires_grad_(True)
		output = nn(input)
...
class LinearSolverTest(metaclass=ABCMeta):
	...
	def test_batches_cpu_n100_float32(self):
		self.solve(100, torch.float32, _cpu, 1)
		self.solve(100, torch.float32, _cpu, 10)
		self.solve(100, torch.float32, _cpu, 100)
		self.solve(100, torch.float32, _cpu, 1000)
...
class Test_GMRES_Solver(LinearSolverTest):
	def solve(self, n, dtype, device, batches=_batches):
		for system in self.systems:
			A, x0, b = get_system(system, n, dtype, device, batches)
			sol, error, iters, flag = linsolve( A, x0, b, method='gmres', tol=3.e-6)
