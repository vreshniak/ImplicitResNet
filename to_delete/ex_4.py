...
if __name__ == '__main__':
	...
	if args.prefix is not None:
		file_name = 'ex_4/'+args.prefix+'_seed_'+str(seed)+'_theta_'+str(theta)+'_h_'+str(step_size)+'_lr_'+str(learning_rate)+\
					'_levels_'+str(num_levels)+'_layers_'+str(num_layers)+'_filters_'+str(num_filters)+'_batch_'+str(batch_size)
	else:
		file_name = 'ex_4/seed_'+str(seed)+'_theta_'+str(theta)+'_h_'+str(step_size)+'_lr_'+str(learning_rate)+\
					'_levels_'+str(num_levels)+'_layers_'+str(num_layers)+'_filters_'+str(num_filters)+'_batch_'+str(batch_size)
	...
	def get_model(theta, writer, seed=None):
		...
		model = Sequential(	Conv2d(im_size[0],num_filters,kernel_size=3,padding=1,bias=False),
